...
>- To find help on a function or package that you have already installed, go to the ***Help*** tab on the bottom right and seach for a package name or function in the seach box
...
<strong class="red2">WARNING:</strong> When using group_by() with summarise() the groups get unwounded after the summarise(). That means if you group by Var1 and Var2 after doing a summary the data frame will only be grouped by Var1. Thus the order of the variables used in the  group_by() matter.
